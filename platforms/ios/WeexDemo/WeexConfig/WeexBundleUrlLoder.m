...
- (NSURL *)jsBundleURL
{
    ...
    if (self.settings[@"launch_locally"] && [self.settings[@"launch_locally"] boolValue]) {
        ...
        if (hostAddress && ![hostAddress isEqualToString:@""]) {
            jsBundleUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@:12588/dist/native/index.js",hostAddress]];
        }else {
            jsBundleUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@:12588/native/index.js", [self getPackageHost]]];
        }
    }
    return jsBundleUrl;
}
