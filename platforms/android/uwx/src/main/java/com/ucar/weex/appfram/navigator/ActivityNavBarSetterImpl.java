...
public class ActivityNavBarSetterImpl implements UWXActivityNavBarSetter {
    @Override
    ...
    @Override
    public boolean setNavBar(FrameLayout navBarView, UWXBundleInfo.NavBar navBar) {
        if (navBar == null) {
            return false;
        }
        UWLog.v("ActivityNavBarSetterImpl",JSON.toJSONString(navBar));
        UWXNavBar uNavBar = new UWXNavBar(navBarView.getContext());
