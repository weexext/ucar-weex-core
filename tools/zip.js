...
if (process.argv[2]) {
  if (process.argv[2] === 'ios') { // ios
    manifast.versionCode =Number(manifast.versionNameIos.replace(/\./g,''));
  } else if(process.argv[2] === 'android') { // android
    manifast.versionCode =Number(manifast.versionNameAndroid.replace(/\./g,''));
  }
} else {
  if (manifast.versionNameAndroid === manifast.versionNameIos) {
    manifast.versionCode =Number(manifast.versionNameAndroid.replace(/\./g,''));
  }else {
    console.error('versionNameIos & versionNameAndroid 不一致，请选择你要打的指定平台。[ios,android]')
    return;
  }
}
