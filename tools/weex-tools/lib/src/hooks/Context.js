...
Context.prototype.requireCordovaModule = function (modulePath) {
    ...
    // using 'cordova-src/...' path.
    // This path will be resolved only when running cordova from 'normal' installation
    // (without symlinked modules). If cordova-src linked to cordova-cli this path is
    // never resolved, so hook fails with 'Error: Cannot find module 'cordova-src''
    var resolvedPath = path.resolve(__dirname, modulePath.replace(/^cordova-lib/, '../../../cordova-src'));
