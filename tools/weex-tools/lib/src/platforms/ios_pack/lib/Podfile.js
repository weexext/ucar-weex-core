...
function Podfile(podFilePath, projectName) {
    ...
    if (!fs.existsSync(this.path)) {
        ...
        this.pods = this.__parseForPods(file);
        this.source = this.__parseForSource(file)
    }
}

Podfile.prototype.__parseForSource = function(text) {
    // split by \n
    var arr = text.split('\n');
...
Podfile.prototype.getPodfileTemplateForAli = function() {
    return util.format(
...
Podfile.prototype.writeUseAliSource = function() {
    ...
    this.source = [
        'git@gitlab.alibaba-inc.com:alipods/specs.git',
        'git@gitlab.alibaba-inc.com:alipods/specs-mirror.git'
    ]

    if(this.source){
        for(var i=0; i<this.source.length; i++){
            sourceString += 'source \'' + this.source[i] + '\'\n'
        }
    }
